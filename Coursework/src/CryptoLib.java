...
public class CryptoLib {
	 ...
	 */
	public static SecretKey createDESKey(byte[] keyData) throws InvalidKeySpecException, NoSuchAlgorithmException, InvalidKeyException
	{
		if(keyData.length != 8){
	...
	/**
	 * Create a new AES key from a given byte array. 
	 * You should use this whenever you need to create a AES key from a particular array of bytes.
	 * @param keyData An array of bytes to be used as a key. This is expected to be an array of 16 bytes.
	 * @return A AES key computed from the given byte array.
	 */
	public static SecretKey createAESKey(byte[] keyData) throws InvalidKeySpecException, NoSuchAlgorithmException, InvalidKeyException
	{
		if(keyData.length != 16){
			...
			SecretKey skey = new SecretKeySpec(keyData, "AES");
            return skey;
		}
	}
